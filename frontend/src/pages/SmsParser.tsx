...
const steps = [
  {
    icon: <svg viewBox="0 0 24 24" className="h-5 w-5" fill="none" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" /></svg>,
    title: "Paste SMS",
    desc: "Copy a transaction message from your banking app",
  },
  {
    icon: <svg viewBox="0 0 24 24" className="h-5 w-5" fill="none" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>,
    title: "AI Parses",
    desc: "ML model extracts amount, merchant and category",
  },
  {
    icon: <svg viewBox="0 0 24 24" className="h-5 w-5" fill="none" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>,
    title: "Auto-Add",
    desc: "Record the expense instantly with one tap",
  },
];
...
const SmsParser = () => {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await addExpense({ amount: parseFloat(result.amount), category: selectedCategory, description: `SMS: ${result.merchant}` });
      push("Expense saved!", "success");
      setResult(null);
      setSmsText("");
    } catch (err: any) {
  ...
  return (
    <div className="space-y-5 stagger">
      <div>
        ...
        <p className="text-sm" style={{ color: "var(--text-secondary)" }}>Parse manual SMS and import your last 90 days bank SMS from one place</p>
      </div>

      {isCapacitor && (
        <div className="card space-y-3">
          <p className="text-sm font-semibold" style={{ color: "var(--text-primary)" }}>Auto SMS Import (Android APK)</p>

          {!hasPermission ? (
            <button onClick={requestPermissions} className="btn-secondary w-full" type="button">Allow SMS Access</button>
          ) : (
            <>
              {debugSummary && <p className="text-xs" style={{ color: "var(--text-secondary)" }}>{debugSummary}</p>}
              {isImporting ? (
                <div>
                  ...
                  <div className="progress-track">
                    <div className="progress-bar" style={{ width: `${importProgress.total > 0 ? (importProgress.done / importProgress.total) * 100 : 0}%` }} />
                  </div>
                </div>
              ) : (
                <button onClick={importInboxHistory} className="btn-secondary w-full" type="button">Import Last 90 Days</button>
              )}
            </>
          )}
        </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        {steps.map((s, i) => (
          <div key={i} className="card !p-4 flex flex-col items-center text-center">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl mb-3 text-white" style={{ background: "var(--gradient-primary)" }}>{s.icon}</div>
          ...
          <div className="mb-4">
            ...
            <div className="flex flex-wrap gap-1.5">
              {categories.map((cat) => {
                ...
                return (
                  <button key={cat} onClick={() => setSelectedCategory(cat)} className="cat-chip" style={active ? { background: `${color}18`, borderColor: color, color } : {}}>
                    {cat}
                  </button>
                );
              })}
